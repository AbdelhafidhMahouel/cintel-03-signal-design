"""
signal_design_abdelhafidh.py - Project script (custom).
...
- Added a new binary signal called high_error_flag.
- This flag is 1 when error_rate > 0.05, otherwise 0.
- This helps identify rows with unusually high error rates.
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/signals_abdelhafidh.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.signal_design_abdelhafidh
"""
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_abdelhafidh.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.7: DEFINE THE HIGH ERROR FLAG SIGNAL RECIPE
    # ----------------------------------------------------
    # This custom signal creates a binary flag:
    # 1 when error_rate is greater than 5%
    # 0 otherwise
    # Name the new column "high_error_flag".
    high_error_flag_signal_recipe: pl.Expr = (
        pl.when(is_requests_positive)
        .then(pl.when(calculated_error_rate > 0.05).then(1).otherwise(0))
        .otherwise(0)
